...
if (item_count > 0) {
    ...
    for (var i = start_count; i != end_count; i++) {
        ...
        if (global.hudState == "shop") {
            ...
            draw_sprite(spr_inv_item_count_bg, 0,view_xview+228, viewY+Ypos);
            //Item price
            draw_text(view_xview+207,viewY+Ypos, string(item_price_draw[i]) + "  G");*/
        } else {
			if (!global.hudState == "inventoryEquipments") {
	            //Item amount background
	            draw_sprite(spr_inv_item_count_bg, 0,viewX+228, viewY+Ypos);
	            //Item amount
            ...
            global.hudState == "shop")
        {
            ...
            draw_sprite(spr_inv_item_count_bg, 0,viewX+295, viewY+Ypos);
            //Item weight
